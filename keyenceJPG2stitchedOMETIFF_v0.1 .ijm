...
tile1 = Array.filter(ALLimagelist, "(_00001_CH[1234].jpg)");
...
tileimage_list = Array.filter(ALLimagelist, "(_[0-9]{5}_" + ch_list[0] + ".jpg)");
...
for (i = 0; i < (tile_count); i++) {
	split1 = split(tileimage_list[i], ".jpg");
	tile1_name = split1[split1.length - 1];
	split2 = split(tile1_name, "_");
...
for (i = 0; i < (tile_count); i++) {
	...
	for (j = 0; j < (ch_count); j++) {
		openimage_list = Array.filter(ALLimagelist, tile_list[i] + "_" + ch_list_new[j] + ".jpg");
		openimage_list_tif = replace(openimage_list[0], ".jpg", ".tif");
		...
		if (image_format == "HE") {
			...
			stack2gray();
			run("Bio-Formats Exporter", 
				"save=[" + tmppath + File.separator + openimage_list_tif + "]" +
				" export compression=Uncompressed");
		}
	}
	close("*");
