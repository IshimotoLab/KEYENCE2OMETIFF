...
if (ch_count == 1) {
	...
	items1 = newArray("x2", "x4", "x10", "x20", "x40", "x60");
...
ch_name_list_new = newArray(ch_count);

for (i = 0; i < (ch_count); i++) {
	chindex = index(ch_list, ch_list_new[i]);
	ch_name_list_new[i] = ch_name_list[chindex];
}
...
run("BigStitcher", "select=define " +
	...
	" keep_metadata_rotation how_to_store_input_images=[Load raw data directly (no resaving)]" +
	" load_raw_data_virtually" +
	" metadata_save_path=[" + tmppath + "]" +
	" image_data_save_path=[" + tmppath + "]" +
	" check_stack_sizes");
...
savepath = saveDir + samplename + "_" + String.join(ch_name_list_new, "_");
...
function calcKeyenceCalib(m, res) { 
	magnif_rate_arr = newArray(1, 2, 5, 10, 20, 30);
	calib_arr = newArray(11.32325, 7.54883, 3.77441);
